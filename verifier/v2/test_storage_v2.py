...
# drafts lane carries an AUTOSAVE copy (autosave: true) by emmawong — newer than
# the published lane by rchu. "Open" must serve the autosaved draft (latest saved
# version), and the author of the latest version must be visible in the editor.
CLOUD = {
    "drafts_version": {"publishedAt": "2026-08-25T08:00:00Z", "publishedBy": "emmawong@mohg.com", "stage": "draft", "autosave": True},
    "published_version": {"publishedAt": "2026-08-20T08:00:00Z", "publishedBy": "rchu@mohg.com", "stage": "published"},
...
with sync_playwright() as p:
    ...
    check("P1 Open serves autosaved draft (published or not)", "DRAFT MARKER TITLE" in title
          and CLOUD["drafts_version"].get("autosave") is True)
    ...
    # ---- E3: author attribution — latest version by emmawong loads and is named ----
    ctx, page, errors = new_page(browser)
    CLOUD["drafts_version"] = {"publishedAt": "2026-08-25T09:00:00Z", "publishedBy": "emmawong@mohg.com", "stage": "draft", "autosave": True}
    CLOUD["published_version"] = {"publishedAt": "2026-08-24T09:00:00Z", "publishedBy": "rchu@mohg.com", "stage": "published"}
    CLOUD["drafts_pb"] = make_pb("EMMAWONG LATEST SAVE")
    CLOUD["published_pb"] = make_pb("RCHU OLDER PUBLISH", with_string_blurb=False)
    page.goto(BASE + "/authoring-tool/?edit=" + SLUG, wait_until="domcontentloaded")
    page.wait_for_function("document.querySelector('#docName') && document.querySelector('#docName').value.length > 0", timeout=20000)
    v = page.eval_on_selector("#docName", "el => el.value")
    check("E3 latest version loads regardless of author", v == "EMMAWONG LATEST SAVE", "docName=" + v)
    try:
        page.wait_for_function("document.querySelector('#toasts') && document.querySelector('#toasts').innerText.includes('emmawong@mohg.com')", timeout=6000)
        author_shown = True
    except Exception:
        author_shown = False
    check("E3 author of latest version is visible", author_shown)
    ctx.close()

    # ---- E4: unsigned local edits are NOT accepted — cloud wins the load ----
    ctx, page, errors = new_page(browser)
    CLOUD["drafts_version"] = {"publishedAt": "2026-08-20T08:00:00Z", "publishedBy": "emmawong@mohg.com", "stage": "draft"}
    CLOUD["drafts_pb"] = make_pb("TITLE V1 CLOUD")
    CLOUD["published_version"] = None
    CLOUD["published_pb"] = None
    page.goto(BASE + "/authoring-tool/?edit=" + SLUG, wait_until="domcontentloaded")
    page.wait_for_function("document.querySelector('#docName') && document.querySelector('#docName').value.length > 0", timeout=20000)
    # Unsigned local edit (no session in this test browser)
    page.eval_on_selector("#docName", "el => { el.value = 'LOCAL UNSIGNED EDIT'; el.dispatchEvent(new Event('input', {bubbles:true})); }")
    page.wait_for_timeout(2500)  # local autosnapshot written, newer than cloud
    page.goto(BASE + "/authoring-tool/?edit=" + SLUG, wait_until="domcontentloaded")
    page.wait_for_timeout(6000)
    v = page.eval_on_selector("#docName", "el => el.value")
    check("E4 unsigned local edit does not become the loaded version", v == "TITLE V1 CLOUD", "docName=" + v)
    ctx.close()
